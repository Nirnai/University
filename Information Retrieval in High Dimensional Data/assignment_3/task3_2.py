...
# CHANGE HERE:
sigmas = [1,0.5,0.1,0.01] # parameter used in the Gaussian kernel
for sigma in sigmas:
    ...
    H = np.outer(y,y)*np.exp(-(squareform(pdist(X)))/(2*sigma**2))
    ...
    b = np.mean(np.sum(lambd[np.where(idx==True)]*y[np.where(idx==True)]*H[np.where(idx==True),np.where(idx==True)],axis = 1)) 
