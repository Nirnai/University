...
int main(int argc, char** argv)
{
    ...
    while(ros::ok())
    {
        ...
        //GET THE VALUES FROM THE TERMINAL AND SAVE THEM IN A LOCAL VARIABLE. YOU WILL GET X,Y AND THETA
        double x = atoll(&myString[0]);
        double y = atoll(&myString[2]);
        double theta = atoll(&myString[4]);

        //CREATE THE MESSAGE WITH THE LOCAL VARIABLE
        msg.request.x = x;
        msg.request.y = y;
        msg.request.theta = theta;


        //COMPUTE THE POSITION AND ORIENTATION OF THE TF FOR THE DESIRED POSITION
        ...
        if(client.call(msg))
        {
            ROS_INFO("Success! Called the service 'TurtlePose'");
        }
        else
        {
